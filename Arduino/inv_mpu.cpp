...
static struct chip_cfg_s temp_chip_cfg_struct = { 0 };

static struct gyro_state_s st = {
    .reg = &reg,
    .hw = &hw,
    .chip_cfg = temp_chip_cfg_struct,
    .test = &test
};
#elif defined MPU6500
const struct gyro_reg_s reg = {
...
#define LOAD_CHUNK  (2)
    ...
    for (ii = 0; ii < length; ii += this_write) {
        ...
        pgmread = pgm_read_word_near(firmware + ii);
        if (mpu_write_mem(ii, this_write, (unsigned char*)&pgmread))
            return -1;
        if (mpu_read_mem(ii, this_write, cur))
            return -1;
        if (memcmp(&pgmread, cur, this_write))
            return -2;
    }
